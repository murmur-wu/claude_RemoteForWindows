...
def _parse_int_set(raw: str) -> set[int]:
    ...
    for piece in raw.split(","):
        ...
        try:
            ...
            raise SystemExit(f"ALLOWED_USER_IDS contains non-integer: {piece!r}")
    return out
...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        cfg = cls(
            discord_token=token,
            allowed_user_ids=_parse_int_set(os.environ.get("ALLOWED_USER_IDS", "")),
            projects=projects,
            default_project=default_project,
            permission_mode=os.environ.get("PERMISSION_MODE", "auto").strip(),
